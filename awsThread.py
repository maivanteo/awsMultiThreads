...
def commit_instanceThread():
    global lockFunc
    lockFunc=1 # lock the function.
    global runningInstances,occupiedInstancesThreads
    hostIP=runningInstances[0]
    for instance in runningInstances:
	instanceIndex=0
	hostIP=instance
	if occupiedInstancesThreads[instanceIndex]==0:
	    occupiedInstancesThreads[instanceIndex]=1
	    break
...
def create_key_pairs_all_regions():
	...
	try:
	    ...
	    pass # if there is an exception, move to the next region and create the next key pair.
...
def start_func(regionRand):
    #create the main program
    global currentImage, lockFunc
    regionIndex=0
    regionIndexBefore=-1
    regionlistLength=len(regionList)
    #while(True):
	...
	time.sleep(6)
        #create_Multiinstances(instancesRandNumber) #create number of instances
        print 'commit'
	while(lockFunc==1): # if function is locked by other thread - sleep for 2 seconds until function release.
	    time.sleep(2)
        commit_instanceThread() # check which instances are running and execute Automain code in it. check thread and instance - occupy or not.
        regionIndexBefore=regionRand # save the last region index number
        #time.sleep(get_random(480,700))
...
def createThreads(regionRand):
	...
	t=threading.Thread(target=start_func,args=[regionRand])
	threads.append(t)
	t.start()
    for thr in threads:
	thr.join()
